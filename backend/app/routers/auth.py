...
async def register(user_data: UserCreate, db: Session = Depends(get_db)):
    ...
    logger.info(
        f"User created successfully: ID={new_user.id}, Email={new_user.email}, role={new_user.role}"
    )
    return new_user
...
async def login(user_data: UserLogin, db: Session = Depends(get_db)):
    ...
    logger.info(
        f"User logged in successfully: ID={user.id}, Email={user.email}, role={user.role}"
    )
    return {
